...
expression returns [Expression ast]:
          ...
          | e1=expression op=('*'|'/'|'%') e2=expression
           {$ast = ArithmeticExpression.create($e1.ast.getLine(), $e1.ast.getColumn(), $e1.ast, $op.text, $e2.ast);}
          | e1=expression op=('+'|'-') e2=expression
...
type returns [Type ast]:
    t1=primitive_type {$ast = $t1.ast;}
    | t2=type '['ic=INT_CONSTANT']' //{$ast = new ArrayType($t2.ast,LexerHelper.lexemeToInt($ic.getText()));}//Factory
    {$ast = ArrayType.create($t2.ast,LexerHelper.lexemeToInt($ic.getText()));}//Factory
